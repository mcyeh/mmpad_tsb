...
from mmpad_submission.wrapper_mmp_ad import MMPAD_HP_U_0, MMPAD_HP_U_1

if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Generating anomaly scores with MMPAD on univariate data')
    ...
    parser.add_argument('--backend', dest='backend',
                        type=str, default='cpu', choices=['cpu', 'gpu'])
    parser.add_argument('--gpu_precision', dest='gpu_precision',
                        type=str, default='float64', choices=['float64', 'float32'])
    parser.add_argument('--gpu_execution', dest='gpu_execution',
                        type=str, default='auto', choices=['auto', 'gpu_pipeline', 'cpu_reference'])
    parser.add_argument('--gpu_device', dest='gpu_device',
                        type=str, default=None)
    parser.add_argument('--gpu_reseed_period', dest='gpu_reseed_period',
                        type=int, default=None)
    parser.add_argument('--gpu_allow_tf32', dest='gpu_allow_tf32',
                        action='store_true')
    parser.add_argument('--verbose', action='store_true')
    args = parser.parse_args()

    if args.hp_version == 0:
        mmp_ad_hp = MMPAD_HP_U_0
    elif args.hp_version == 1:
        mmp_ad_hp = MMPAD_HP_U_1
